<ul class="nav nav-tabs nav-justified mb-3">
    ...
    <li class="nav-item"><a href="{{ route('users.book_lists',['id' => $user->id]) }}" class="text-muted nav-link {{ Request::is('users/*/book_lists') ? 'active' : '' }}">to read book list <span class ="badge badge-secondary">{{ $count_book_lists }}</span></a></li>
    @if(Auth::id() == $user->id)
